...
namespace POS
{
    public partial class M_POS_PURCHASE_SEARCH02 : POSFormBase
    {
        ...
        public M_POS_PURCHASE_SEARCH02()
        {
            InitializeComponent();
            InitializeControl();
            InitEvent();
        ...
        private void PrintThemalAll()
        {
            ...
            for (int i = 0; i < gridView1.RowCount; i++)
            {
                ...
                sb.Append("구매금액 : " + A.GetNumericString(gridView1.GetRowCellValue(i, gridView1.Columns["AM"])).PadLeft(31, ' ') + "\n");
                sb.Append("현금납입 : " + A.GetNumericString(gridView1.GetRowCellValue(i, gridView1.Columns["AM_PAY"])).PadLeft(31, ' ') + "\n");
                if (cfgUseCreditCard.ConfigValue == "Y")
                    sb.Append("카드납입 : " + A.GetNumericString(gridView1.GetRowCellValue(i, gridView1.Columns["AM_CREDIT"])).PadLeft(31, ' ') + "\n");
                sb.Append("미납금액 : " + A.GetNumericString(gridView1.GetRowCellValue(i, gridView1.Columns["AM_NONPAID"])).PadLeft(31, ' ') + "\n");
                sb.Append("------------------------------------------\n");
            }
        ...
        {
            POSPrintHelper.POSReportPrint("R_POS_PURCHASE_SEARCH02_RPT01", new string[] { "CD_STORE", "DT_FROM", "DT_TO" }, new string[] { POSGlobal.StoreCode, dtpFrom.Text, dtpTo.Text });
        }
        #endregion 프린트 모음
