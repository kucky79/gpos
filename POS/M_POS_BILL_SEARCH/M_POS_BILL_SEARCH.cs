...
namespace POS
{
    public partial class M_POS_BILL_SEARCH : POSFormBase
    {
        ...
        private void GridView_RowCellStyle(object sender, RowCellStyleEventArgs e)
        {
            ...
            if (A.GetString(view.GetRowCellValue(e.RowHandle, "FG_PO")) == "R")
            {
        ...
        private void GridView1_CellMerge(object sender, DevExpress.XtraGrid.Views.Grid.CellMergeEventArgs e)
        {
            ...
            else if (e.Column.FieldName == "DT_PO")//Name 컬럼만 Merge
            {
                var dr1 = view.GetDataRow(e.RowHandle1); //위에 행 정보
                var dr2 = view.GetDataRow(e.RowHandle2); //아래 행 정보

                e.Merge = dr1["DT_PO"].ToString().Equals(dr2["DT_PO"].ToString()) && dr1["NM_CUST"].ToString().Equals(dr2["NM_CUST"].ToString());
            }
            else
            {
                e.Merge = false;
        ...
        private void InitializeControl()
        {
            ...
            gridView2.Columns["AM_PAY"].SummaryItem.DisplayFormat = "{0:##,##0.####}";

            gridView2.Columns["AM_CREDIT"].Summary.Add(DevExpress.Data.SummaryItemType.Sum);
            gridView2.Columns["AM_CREDIT"].SummaryItem.DisplayFormat = "{0:##,##0.####}";


            //GridHelper.SetDecimalPoint(gridMain, new string[] { "AM_OPEN", "AM_SALE", "AM_PAY", "AM_CREDIT", "AM_NONPAID" }, 0);
            gridView1.Columns["DT_PO"].ColumnEdit = GridHelper.SetGridMask(Bifrost.CommonFunction.MaskType.DATE);
        ...
        private void PrintThemal()
        {
            ...
            for (int i = 0; i < gridView1.RowCount; i++)
            {
                ...
                SaleDt = A.GetString(gridView1.GetRowCellValue(i, gridView1.Columns["DT_PO"]));
                ...
                sb.Append("이전미납 : " + A.GetNumericString(gridView1.GetRowCellValue(i, gridView1.Columns["AM_OPEN"])).PadLeft(blankLen, ' ') + "\n");
                sb.Append("구매금액 : " + A.GetNumericString(gridView1.GetRowCellValue(i, gridView1.Columns["AM_SALE"])).PadLeft(blankLen, ' ') + "\n");
                sb.Append("현금납입 : " + A.GetNumericString(gridView1.GetRowCellValue(i, gridView1.Columns["AM_PAY"])).PadLeft(blankLen, ' ') + "\n");
                sb.Append("신용카드 : " + A.GetNumericString(gridView1.GetRowCellValue(i, gridView1.Columns["AM_CREDIT"])).PadLeft(blankLen, ' ') + "\n");
                sb.Append("미납금액 : " + A.GetNumericString(gridView1.GetRowCellValue(i, gridView1.Columns["AM_NONPAID"])).PadLeft(blankLen, ' ') + "\n");
                sb.Append("------------------------------------------\n");
            }
        ...
        private void PrintThemalDetail()
        {
            ...
            for (int i = 0; i < gridView2.RowCount; i++)
            {
                ...
                {
                    sb.Append("------------------------------------------\n");
                    sb.Append("이전미납 : " + A.GetNumericString(gridView2.GetRowCellValue(i, gridView2.Columns["AM_OPEN"])).PadLeft(blankLen, ' ') + "\n");
                    sb.Append("구매금액 : " + A.GetNumericString(gridView2.GetRowCellValue(i, gridView2.Columns["AM_SALE"])).PadLeft(blankLen, ' ') + "\n");
                    sb.Append("현금납입 : " + A.GetNumericString(gridView2.GetRowCellValue(i, gridView2.Columns["AM_PAY"])).PadLeft(blankLen, ' ') + "\n");
                    sb.Append("신용카드 : " + A.GetNumericString(gridView2.GetRowCellValue(i, gridView2.Columns["AM_CREDIT"])).PadLeft(blankLen, ' ') + "\n");
                    sb.Append("미납금액 : " + A.GetNumericString(gridView2.GetRowCellValue(i, gridView2.Columns["AM_NONPAID"])).PadLeft(blankLen, ' ') + "\n");
                    sb.Append("******************************************\n");
                    sb.Append(PrinterCommand.NewLine);
                }
            }
        ...
        private void PrintNormal()
        {
            ...
            POSPrintHelper.POSReportPrint("R_POS_BILL_SEARCH01", new string[] { "CD_STORE", "DT_FROM", "DT_TO", "CD_CUST", "YN_RETURN", "FG_SORT" }, strParams);
        }
        ...
        private void PrintNormalDetail()
        {
            ...
            POSPrintHelper.POSReportPrint("R_POS_BILL_SEARCH02", new string[] { "CD_STORE", "DT_FROM", "DT_TO", "CD_CUST", "YN_RETURN", "FG_SORT" }, strParams);
        }
