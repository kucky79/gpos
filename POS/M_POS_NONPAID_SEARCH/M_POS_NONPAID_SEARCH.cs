...
namespace POS
{
    public partial class M_POS_NONPAID_SEARCH : POSFormBase
    {
        DataTable _dtCust = new DataTable();
        Bifrost.Helper.POSConfig cfgUseCreditCard;

        public M_POS_NONPAID_SEARCH()
        {
            InitializeComponent();
            InitControl();
            InitEvent();
        ...
        public override void OnView()
        {
            ...
            _dtCust = Search(new object[] { POSGlobal.StoreCode, string.Empty, rboSearch.EditValue, dtpSearch.Text });
        ...
        {
            POSPrintHelper.POSReportPrint("R_POS_NONPAID_SEARCH", new string[] { "P_CD_STORE", "P_SEARCH", "P_YN_USE", "P_DT_SALE" }, new string[] { POSGlobal.StoreCode, string.Empty, A.GetString(rboSearch.EditValue), dtpSearch.Text });
        ...
        private void PrintNormalPay()
        {
            if (cfgUseCreditCard.ConfigValue == "Y")
            {
                POSPrintHelper.POSReportPrint("R_POS_NONPAID_SEARCH_PAY", new string[] { "CD_STORE", "DT_FROM", "DT_TO", "CD_CUST" }, new string[] { POSGlobal.StoreCode, dtpFrom.Text, dtpTo.Text, A.GetString(gridViewItem.GetFocusedRowCellValue("CD_CUST")) });
            }
            else
            {
                POSPrintHelper.POSReportPrint("R_POS_NONPAID_SEARCH_PAY_WO_CREDIT", new string[] { "CD_STORE", "DT_FROM", "DT_TO", "CD_CUST" }, new string[] { POSGlobal.StoreCode, dtpFrom.Text, dtpTo.Text, A.GetString(gridViewItem.GetFocusedRowCellValue("CD_CUST")) });
            }
        }
        ...
        private void PrintThemalPay()
        {
            ...
            for (int i = 0; i < gridViewDetail.RowCount; i++)
            {
                ...
                if (cfgUseCreditCard.ConfigValue == "Y")
                {
                    SaleDt = A.GetString(gridViewDetail.GetRowCellValue(i, gridViewDetail.Columns["DT_CLS"]));
                    ...
                    sb.Append(PrinterCommand.NewLine);
                    sb.Append("수 금 합 : " + A.GetNumericString(gridViewDetail.GetRowCellValue(i, gridViewDetail.Columns["AM_PAY_TOT"])).PadLeft(31, ' '));
                    sb.Append(PrinterCommand.NewLine);
                    sb.Append("------------------------------------------\n");
