...
namespace POS
{
    public partial class M_POS_PO_SEARCH01 : POSFormBase
    {
        POSConfig cfgUseCreditCard;
        POSConfig cfgVatYN;

        public M_POS_PO_SEARCH01()
        {
            InitializeComponent();
            InitControl();
            InitEvent();
        ...
        private void PrintThemalCredit()
        {
            ...
            for (int i = 0; i < gridViewItem.RowCount; i++)
            {
                ...
                //if (CardAmt > 0)
                {
                    ...
                    sb.Append("매입금액 : " + A.GetNumericString(gridViewItem.GetRowCellValue(i, gridViewItem.Columns["AM_TOT"])).PadLeft(31, ' ') + "\n");
        ...
        {
            POSPrintHelper.POSReportPrint("R_POS_PO_SEARCH01", new string[] { "CD_STORE", "DT_PO", "CD_CUST" }, new string[] { POSGlobal.StoreCode, dtpSearch.Text, A.GetDatatableToString(aLookUpEditCust.Properties.DataSource) });
        }

        private void PrintNormalCredit()
        {
            POSPrintHelper.POSReportPrint("R_POS_PO_SEARCH01_CREDIT", new string[] { "CD_STORE", "DT_PO", "CD_CUST" }, new string[] { POSGlobal.StoreCode, dtpSearch.Text, A.GetDatatableToString(aLookUpEditCust.Properties.DataSource) });
        }

        private void PrintNormalVat()
        {
            POSPrintHelper.POSReportPrint("R_POS_PO_SEARCH01_VAT", new string[] { "CD_STORE", "DT_PO", "CD_CUST" }, new string[] { POSGlobal.StoreCode, dtpSearch.Text, A.GetDatatableToString(aLookUpEditCust.Properties.DataSource) });
        }
