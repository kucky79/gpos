...
namespace POS
{
    public partial class P_POS_SALE_TMP : POSPopupBase
    {
        ...
        {
            string SlipNo = A.GetString(gridView1.GetFocusedRowCellValue("NO_SO"));
            ...
            if (ShowMessageBoxA(CustomerName + " 거래처의 임시저장건을 삭제하시겠습니까?", MessageType.Question) == DialogResult.Yes)
            {
                ...
                if (OrderType == TempOrderType.Order)
                    result = DBHelper.ExecuteNonQuery("USP_POS_SO_TMP_D", new object[] { POSGlobal.StoreCode, SlipNo });
                else if (OrderType == TempOrderType.Customer)
                    result = DBHelper.ExecuteNonQuery("USP_POS_SO_TMP_CUST_D", new object[] { POSGlobal.StoreCode, CustomerCode, dtpSale.Text });
