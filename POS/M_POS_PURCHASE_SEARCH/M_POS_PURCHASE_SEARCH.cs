...
namespace POS
{
    public partial class M_POS_PURCHASE_SEARCH : POSFormBase
    {
        ...
        private void InitEvent()
        {
            ...
            gridView2.CustomSummaryCalculate += GridView_CustomSummaryCalculate;
            gridView3.CustomSummaryCalculate += GridView_CustomSummaryCalculate;
            gridView4.CustomSummaryCalculate += GridView_CustomSummaryCalculate;
        ...
        private void GridView1_CellMerge(object sender, CellMergeEventArgs e)
        {
            ...
            if (e.Column.FieldName == "DT_PURCHASE")//Name 컬럼만 Merge
            {
                var dr1 = view.GetDataRow(e.RowHandle1); //위에 행 정보
                var dr2 = view.GetDataRow(e.RowHandle2); //아래 행 정보

                e.Merge = dr1["DT_PURCHASE"].ToString().Equals(dr2["DT_PURCHASE"].ToString()) && dr1["NM_CUST"].ToString().Equals(dr2["NM_CUST"].ToString());
            }
        ...
        private void GridView2_CellMerge(object sender, DevExpress.XtraGrid.Views.Grid.CellMergeEventArgs e)
        {
            ...
            if (e.Column.FieldName == "DT_PO")//Name 컬럼만 Merge
            {
                var dr1 = view.GetDataRow(e.RowHandle1); //위에 행 정보
                var dr2 = view.GetDataRow(e.RowHandle2); //아래 행 정보

                e.Merge = dr1["NO_PO"].ToString().Equals(dr2["NO_PO"].ToString());
            }
            else if (e.Column.FieldName == "NM_CUST")//Name 컬럼만 Merge
            {
                var dr1 = view.GetDataRow(e.RowHandle1); //위에 행 정보
        ...
        private void BtnCust_Click(object sender, EventArgs e)
        {
            ...
            P_POS_CONTENTS_SINGLE.ContentsType = ContentsMode.Customer;
            P_POS_CONTENTS_SINGLE.CustomerType = "1";
        ...
        private void InitializeControl()
        {
            ...
            GridHelper.SetDecimalPoint(gridMain, new string[] { "AM_PURCHASE", "AM_VAT", "AM_TOT" }, 0);
            gridView1.Columns["DT_PURCHASE"].ColumnEdit = GridHelper.SetGridMask(Bifrost.CommonFunction.MaskType.DATE);

            GridHelper.SetDecimalPoint(gridDetail, new string[] { "UM", "AM", "QT_UNIT" }, 0);
            gridView2.Columns["DT_PO"].ColumnEdit = GridHelper.SetGridMask(Bifrost.CommonFunction.MaskType.DATE);

            gridView1.OptionsView.AllowCellMerge = true;
            gridView2.OptionsView.AllowCellMerge = true;


            gridView1.Columns["AM_PURCHASE"].Summary.Add(DevExpress.Data.SummaryItemType.Sum);
            gridView1.Columns["AM_PURCHASE"].SummaryItem.DisplayFormat = "{0:##,##0.####}";
            ...
            gridView3.Columns["AM"].Summary.Add(DevExpress.Data.SummaryItemType.Custom);
            gridView3.Columns["AM"].SummaryItem.DisplayFormat = "{0:##,##0.####}";

            gridView4.Columns["AM"].Summary.Add(DevExpress.Data.SummaryItemType.Custom);
            gridView4.Columns["AM"].SummaryItem.DisplayFormat = "{0:##,##0.####}";
        ...
        public override void OnExcelExport()
        {
            ...
            {
                case 0:
                    GridHelper.ExcelExport(gridMain, "구매현황");
                    break;
                case 1:
                    GridHelper.ExcelExport(gridDetail, "구매현황(상세)");
                    break;
                case 2:
                    GridHelper.ExcelExport(gridItem, "구매현황(상품별)");
                    break;
                case 3:
                    GridHelper.ExcelExport(gridMonth, "구매현황(월별)");
                    break;
                default:
                    break;
            }
        ...
        private void PrintThemal()
        {
            ...
            {
                decimal NonPaidAmt = A.GetDecimal(gridView1.GetRowCellValue(i, gridView1.Columns["AM_TOT"]));

                SaleDt = A.GetString(gridView1.GetRowCellValue(i, gridView1.Columns["DT_PURCHASE"]));
            ...
            decimal SumValue = A.GetDecimal(gridView1.Columns["AM_PURCHASE"].SummaryItem.SummaryValue);
        ...
        private void PrintThemalDayItem()
        {
            ...
            for (int i = 0; i < gridView2.RowCount; i++)
            {
                if (A.GetInt(gridView2.GetRowCellValue(i, gridView2.Columns["NO_ROW"])) == 1)
                {
                    ...
                    SaleDt = A.GetString(gridView2.GetRowCellValue(i, gridView2.Columns["DT_PO"]));
                    SaleDt = SaleDt.Substring(0, 4) + "-" + SaleDt.Substring(4, 2) + "-" + SaleDt.Substring(6, 2);
                    sb.Append(PrinterCommand.AlignLeft);
                ...
                //합계
                if (A.GetString(gridView2.GetRowCellValue(i, gridView2.Columns["NO_ROW"])) == A.GetString(gridView2.GetRowCellValue(i, gridView2.Columns["NO_MAX"])) && A.GetString(aRadioButtonSumYN.EditValue) == "Y")
                {
                    sb.Append("------------------------------------------\n");
            ...
            for (int i = 0; i < gridView3.RowCount; i++)
            {
                if (A.GetString(gridView3.GetRowCellValue(i, gridView3.Columns["NO_ROW"])) != "999")
                {
                    ...
                    if (A.GetString(gridView3.GetRowCellValue(i, gridView3.Columns["CD_ITEM"])) != "99999999999999")
                    {
                        ...
                        //else if (ReceitType == ReceitMode.Single)
                        //{
                        string FirstGroup = A.GetString(gridView3.GetRowCellValue(i, gridView3.Columns["NO_ROW"])).PadLeft(3, ' ') + " " + ItemName;
        ...
        private void PrintThemalMonth()
        {
            ...
            for (int i = 0; i < gridView4.RowCount; i++)
            {
                if (A.GetString(gridView4.GetRowCellValue(i, gridView4.Columns["NO_ROW"])) != "999")
                {
                    ...
                    SaleDt = A.GetString(gridView4.GetRowCellValue(i, gridView4.Columns["DT_PO"]));
        ...
        {
            POSPrintHelper.POSReportPrint("R_POS_PURCHASE_SEARCH01", new string[] { "CD_STORE", "DT_FROM", "DT_TO", "CD_CUST", "FG_SORT" }, new string[] { POSGlobal.StoreCode, dtpFrom.Text, dtpTo.Text, A.GetDatatableToString(aLookUpEditCust.Properties.DataSource), A.GetString(aRadioButtonSort.EditValue) });
        }

        private void PrintNormalDayItem()
        {
            POSPrintHelper.POSReportPrint("R_POS_PURCHASE_SEARCH02", new string[] { "CD_STORE", "DT_FROM", "DT_TO", "CD_CUST", "YN_SUM", "FG_SORT" }, new string[] { POSGlobal.StoreCode, dtpFrom.Text, dtpTo.Text, A.GetDatatableToString(aLookUpEditCust.Properties.DataSource), A.GetString(aRadioButtonSumYN.EditValue), A.GetString(aRadioButtonSort.EditValue) });
        }

        private void PrintNormalItem()
        {
            POSPrintHelper.POSReportPrint("R_POS_PURCHASE_SEARCH03", new string[] { "CD_STORE", "DT_FROM", "DT_TO", "CD_CUST", "YN_SUM", "FG_SORT" }, new string[] { POSGlobal.StoreCode, dtpFrom.Text, dtpTo.Text, A.GetDatatableToString(aLookUpEditCust.Properties.DataSource), A.GetString(aRadioButtonSumYN.EditValue), A.GetString(aRadioButtonSort.EditValue) });
        }

        private void PrintNormalMonth()
        {
            POSPrintHelper.POSReportPrint("R_POS_PURCHASE_SEARCH04", new string[] { "CD_STORE", "DT_FROM", "DT_TO", "CD_CUST", "YN_SUM", "FG_SORT" }, new string[] { POSGlobal.StoreCode, dtpFrom.Text, dtpTo.Text, A.GetDatatableToString(aLookUpEditCust.Properties.DataSource), A.GetString(aRadioButtonSumYN.EditValue), A.GetString(aRadioButtonSort.EditValue) });
        }
        #endregion 프린트 모음
    }
}
