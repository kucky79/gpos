...
namespace POS
{
    public partial class M_POS_OLD03 : POSFormBase
    {
        public M_POS_OLD03()
        {
            InitializeComponent();
        ...
        private void PrintThemal()
        {
            ...
            for (int i = 0; i < gridViewMain.RowCount; i++)
            {
                ...
                sb.Append("------------------------------------------\n");
                sb.Append("이전외상 : " + A.GetNumericString(gridViewMain.GetRowCellValue(i, gridViewMain.Columns["OLDDEF_AMT"])).PadLeft(blankLen, ' ') + "\n");
                sb.Append("판매금액 : " + A.GetNumericString(gridViewMain.GetRowCellValue(i, gridViewMain.Columns["SALE_AMT"])).PadLeft(blankLen, ' ') + "\n");
                sb.Append("수 금 액 : " + A.GetNumericString(gridViewMain.GetRowCellValue(i, gridViewMain.Columns["CLS_AMT"])).PadLeft(blankLen, ' ') + "\n");
                sb.Append("외상금액 : " + A.GetNumericString(gridViewMain.GetRowCellValue(i, gridViewMain.Columns["NEWDEF_AMT"])).PadLeft(blankLen, ' ') + "\n");
                sb.Append("******************************************\n");
                sb.Append(PrinterCommand.NewLine);
            }

            sb.Append("판 매 합 : " + A.GetNumericString(gridViewMain.Columns["SALE_AMT"].SummaryItem.SummaryValue).PadLeft(blankLen, ' ') + "\n");
            sb.Append("수 금 합 : " + A.GetNumericString(gridViewMain.Columns["CLS_AMT"].SummaryItem.SummaryValue).PadLeft(blankLen, ' ') + "\n");
            sb.Append("==========================================\n");
        ...
        private void PrintThemalDetail()
        {
            ...
            for (int i = 0; i < gridViewItem.RowCount; i++)
            {
                ...
                if (A.GetInt(gridViewItem.GetRowCellValue(i, gridViewItem.Columns["NO_ROW"])) != 999)
                {
                    ...
                    sb.Append(PrinterCommand.AlignLeft);
                    sb.Append(RowCnt.ToString().PadLeft(3, ' ') + " " + ItemName);
                    sb.Append(PrinterCommand.NewLine);
                ...
                {
                    sb.Append("------------------------------------------\n");
                    sb.Append("이전외상 : " + A.GetNumericString(gridViewItem.GetRowCellValue(i, gridViewItem.Columns["OLDDEF_AMT"])).PadLeft(blankLen, ' ') + "\n");
                    sb.Append("판매금액 : " + A.GetNumericString(gridViewItem.GetRowCellValue(i, gridViewItem.Columns["SALE_AMT"])).PadLeft(blankLen, ' ') + "\n");
                    sb.Append("수 금 액 : " + A.GetNumericString(gridViewItem.GetRowCellValue(i, gridViewItem.Columns["CLS_AMT"])).PadLeft(blankLen, ' ') + "\n");
                    sb.Append("외상금액 : " + A.GetNumericString(gridViewItem.GetRowCellValue(i, gridViewItem.Columns["NEWDEF_AMT"])).PadLeft(blankLen, ' ') + "\n");
                    sb.Append("******************************************\n");
                    sb.Append(PrinterCommand.NewLine);
                }
            }

            sb.Append("판 매 합 : " + A.GetNumericString(gridViewItem.Columns["SALE_AMT"].SummaryItem.SummaryValue).PadLeft(blankLen, ' ') + "\n");
            sb.Append("수 금 합 : " + A.GetNumericString(gridViewItem.Columns["CLS_AMT"].SummaryItem.SummaryValue).PadLeft(blankLen, ' ') + "\n");
            sb.Append("==========================================\n");
        ...
        {
            POSPrintHelper.POSReportPrintOld("R_POS_OLD03_SUM", new string[] { "CD_STORE", "DT_FROM", "DT_TO", "CD_CUST", "YN_RETURN" }, new string[] { POSGlobal.StoreCode, dtpFrom.Text, dtpTo.Text, A.GetDatatableToString(aLookUpEditCust.Properties.DataSource), aRadioButtonReturnYN.EditValue.ToString() });
        }

        private void PrintNormalDetail()
        {
            POSPrintHelper.POSReportPrintOld("R_POS_OLD03", new string[] { "CD_STORE", "DT_FROM", "DT_TO", "CD_CUST", "YN_RETURN" }, new string[] { POSGlobal.StoreCode, dtpFrom.Text, dtpTo.Text, A.GetDatatableToString(aLookUpEditCust.Properties.DataSource), aRadioButtonReturnYN.EditValue.ToString() });
        }
