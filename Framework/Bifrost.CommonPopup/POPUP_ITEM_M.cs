...
{
    public partial class POPUP_ITEM_M : PopupBase
    {
        ...
        private void InitGrid()
        {
            ...
            gvItem.OptionsSelection.MultiSelect = true;
            gvItem.OptionsSelection.MultiSelectMode = DevExpress.XtraGrid.Views.Grid.GridMultiSelectMode.CheckBoxRowSelect;
            gvItem.OptionsSelection.ResetSelectionClickOutsideCheckboxSelector = false;
        ...
        {
            if(gvItem.SelectedRowsCount > 0)
            {
                int[] selectedRows = gvItem.GetSelectedRows();
                DataRow[] result = new DataRow[selectedRows.Length];

                for(int i = 0; i < selectedRows.Length; i++)
                {
                    int rowHandle = selectedRows[i];

                    if (!gvItem.IsGroupRow(rowHandle))
                    {
                        result[i] = gvItem.GetDataRow(rowHandle);
                    }
                }

                DataTable dtResult = result.CopyToDataTable();

                ReturnData.Add("ReturnData", dtResult);
        ...
        private void BtnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                if(ShowMessageBoxA("전체검색을 하면 시간이 오래 걸릴수 있습니다.\n계속하시겠습니까?", NF.Framework.Common.MessageType.Question) == DialogResult.Yes)
                {
