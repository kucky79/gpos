...
namespace Bifrost.Common
{
    public class DbAgentOld
    {
        ...
        private static DbAgentOld _instance = null;
        ...
        private DbAgentOld()
        {
            ...
            //_connectionString = inifile.IniReadValue("DB", "OldConnectionString", Path);

            //if (_connectionString == "")
            {
                //20200409 강제로 일단 박아둠
                //default 
                inifile.IniWriteValue("DB", "OldConnectionString", encryptor.Encryptor(DesType.Encrypt, "Data Source=localhost; initial Catalog=GPOS_OLD;uid=YOUR_USER;password=YOUR_PASSWORD;"), Path);
                _connectionString = encryptor.Encryptor(DesType.Decrypt, inifile.IniReadValue("DB", "OldConnectionString", Path));
            }
        }

        public DbAgentOld(string connectionString)
        {
            this._connectionString = connectionString;
        }

        protected internal static DbAgentOld GetInstance()
        {
            if (_instance == null)
            {
                _instance = new DbAgentOld();
            }
        ...
        {
            _instance = new DbAgentOld(connectionString);
        }
