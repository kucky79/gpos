...
namespace Bifrost
{
    public class Global : IDisposable
    {
        ...
        private static string _AuthBiz;                     // 사업장권한
        private static string _AuthDept;                    // 부서권한
        private static string _AuthFghire;                     // 사원구분권한
        private static string _AuthHrsbase;                     // 인사기본권한
        ...
        private static string _EmpEnName;	                // 사원명(영문)
        ...
        public Global(DataRow gbDataRow)
        {
            ...
            //메뉴 권한 관련 주석 20190910
            //DataSet ds = DBHelper.GetDataSet("SP_HRS_USERAUTH_S", new object[] { FirmCode, Convert.ToString(gbDataRow["CD_USER"]).Trim() });
            //if (ds.Tables[0].Rows.Count > 0)
            //{
            //    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            //    {
            //        AuthBiz = AuthBiz + Convert.ToString(ds.Tables[0].Rows[i][0]).Trim() + "|";
            //    }
            //    AuthBiz = AuthBiz.Substring(0, AuthBiz.Length - 1);
            //}
            ...
            //    {
            //        AuthDept = AuthDept + Convert.ToString(ds.Tables[1].Rows[i][0]).Trim() + "|";
            //    }
            ...
            //    {
            //        AuthFghire = AuthFghire + Convert.ToString(ds.Tables[2].Rows[i][0]).Trim() + "|";
            //    }
            //    AuthFghire = AuthFghire.Substring(0, AuthFghire.Length - 1);
            //}
            ...
            //    {
            //        AuthHrsbase = AuthHrsbase + Convert.ToString(ds.Tables[3].Rows[i][0]).Trim() + "|";
            //    }
            //    AuthHrsbase = AuthHrsbase.Substring(0, AuthHrsbase.Length - 1);
            //}
            //메뉴 권한 관련 주석 20190910


            //UserData도 Global로 올림. 기존것도 유지
            DeptCode = Convert.ToString(gbDataRow["CD_DEPT"]).Trim();
            DeptName = Convert.ToString(gbDataRow["NM_DEPT"]).Trim();
            ...
            RoleType = Convert.ToString(gbDataRow["FG_ROLE"]).Trim();
            RoleName = Convert.ToString(gbDataRow["NM_FG_ROLE"]).Trim();
            PositionType = Convert.ToString(gbDataRow["FG_POS"]).Trim();
            PositionName = Convert.ToString(gbDataRow["NM_FG_POS"]).Trim();
        ...
        //    ud.EmpEnName = Convert.ToString(userDataRow["NM_EMP_E"]).Trim();
        ...
        public static string AuthFghire
        {
            get { return _AuthFghire; }
            set { _AuthFghire = value; }
        }

        public static string AuthHrsbase
        {
            get { return _AuthHrsbase; }
            set { _AuthHrsbase = value; }
        }
        #endregion
        ...
        {
            get { return _EmpEnName; }
            set { _EmpEnName = value; }
        }
        /// <summary>
        /// 접속ID
        /// </summary>
    ...
    {
        public const string FORMSAUTHKEY = "NFFormsCookieName";
        ...
        public AuthHeader()
        {
            this._Ticket = string.Empty;
            this._LangID = System.Globalization.CultureInfo.CurrentCulture.Name;
        }
