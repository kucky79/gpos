...
namespace POS
{
    public partial class P_POS_CONTENTS : POSPopupBase
    {

        DevExpress.XtraEditors.CheckButton[] _contentsBtn;
        DataTable _dt;
        public Dictionary<string, string> SelectedContent = new Dictionary<string, string>();

        public P_POS_CONTENTS()
        {
            InitializeComponent();
            InitForm();
            InitEvent();
        ...
        //    {
        //        _contentsType = value;
        //        PopupTitle = _contentsType == ContentsMode.Customer ? "고객 도움창" : "상품 도움창";
        //    } 
        //} 
        ...
        private void P_POS_CONTENTS_Load(object sender, EventArgs e)
        {
            OnSearch();
        }
        ...
        {
            _contentsBtn = new DevExpress.XtraEditors.CheckButton[ScreenQty];
        ...
        private void Setbtn(Dictionary<string, string>  selectedBtn)
        {
            for (int i = 0; i < ScreenQty; i++)
            {
                //일단 전부 체크 해제
                ...
                var checkedBtn = selectedBtn.FirstOrDefault(x => x.Key == _contentsBtn[i].Tag.ToString()).Key;
                _contentsBtn[i].Checked = checkedBtn == null ? false : true;
            }
        }
        ...
        private void BtnContents_Click(object sender, EventArgs e)
        {
            ...
            string Code = ((DevExpress.XtraEditors.CheckButton)sender).Tag.ToString();
            string Name = ((DevExpress.XtraEditors.CheckButton)sender).Text;

            switch (ContentsType)
            {
                case ContentsMode.Customer:
                    //이미 추가된 키면 삭제
                    if (SelectedContent.ContainsKey(Code))
                    {
                        SelectedContent.Remove(Code);
                    }
                    else //아니면 추가
                    {
                        SelectedContent.Add(Code, Name);
                    }
                    break;
                case ContentsMode.Item:
                    ...
                    {
                        //일단 체크 해제
                        BeginInvoke(new Action(() => { ((DevExpress.XtraEditors.CheckButton)sender).Checked = false; }));

                        DataColumn[] dtkey = new DataColumn[1];
                        _dt = DBHelper.GetDataTable("USP_POS_ITEM_S", new object[] { POSGlobal.StoreCode, string.Empty, "L", Code });
            ...
            //ResultCode = ((DevExpress.XtraEditors.CheckButton)sender).Tag.ToString();
            //ResultName = ((DevExpress.XtraEditors.CheckButton)sender).Text;
            //this.DialogResult = DialogResult.OK;
        }
        ...
        private void SetContentsPreNext()
        {
            ...
            for (int j = ScreenQty * (CurrentPage - 1); j < ScreenQty * CurrentPage; j++)
            {
                ...
                {
                    _contentsBtn[j % ScreenQty].Text = _dt.Rows.Count > j ? _dt.Rows[j][1].ToString().Replace("\\r\\n", Environment.NewLine) : string.Empty;
                    _contentsBtn[j % ScreenQty].Tag = _dt.Rows.Count > j ? _dt.Rows[j][0].ToString() : string.Empty;
