...
namespace POS
{
    public partial class P_POS_ORDER_TMP : POSPopupBase
    {
        ...
        private void InitEvent()
        {
            ...
            btnTmpItem.Click += BtnTmpItem_Click;
            dtpSale.DateTimeChanged += DtpSale_DateTimeChanged1;
        }

        private void DtpSale_DateTimeChanged1(object sender, EventArgs e)
        {
            OnSearch();
        }

        private void BtnTmpItem_Click(object sender, EventArgs e)
        {
            P_POS_TMP_ITEM P_POS_TMP_ITEM = new P_POS_TMP_ITEM();
            P_POS_TMP_ITEM.SlipDt = dtpSale.Text;
            P_POS_TMP_ITEM.SlipType = SlipType;

            if (P_POS_TMP_ITEM.ShowDialog() == DialogResult.OK)
            {

            }
            if (P_POS_TMP_ITEM != null)
            {
                P_POS_TMP_ITEM.Dispose();
            }
        }

        private void BtnDelete_Click(object sender, EventArgs e)
        {
            string SlipNo = A.GetString(gridView1.GetFocusedRowCellValue("NO_SLIP"));
            ...
            if (ShowMessageBoxA(CustomerName + " 거래처의 임시저장건을 삭제하시겠습니까?", MessageType.Question) == DialogResult.Yes)
            {
                ...
                if (OrderType == TempOrderType.Order)
                    result = DBHelper.ExecuteNonQuery("USP_POS_TO_TMP_BY_ORDER_D", new object[] { POSGlobal.StoreCode, SlipNo, SlipType });
                else if (OrderType == TempOrderType.Customer)
                    result = DBHelper.ExecuteNonQuery("USP_POS_TO_TMP_BY_CUST_D", new object[] { POSGlobal.StoreCode, dtpSale.Text, CustomerCode, SlipType });
        ...
        protected override void OnSearch()
        {
            ...
            _dt = DBHelper.GetDataTable("USP_POS_TO_S", new object[] { POSGlobal.StoreCode, dtpSale.Text, SlipType, CustomerCode, tmpOrderType });
