...
namespace NF.Framework.Win.Validation
{
	...
	public abstract class BaseValidator : Component, ISupportInitialize
	{
		...
		private Color _OrgBackColor = Color.Empty;
		...
		[Category("Appearance")]
		[Description("Sets or returns the background color when error occurs.")]
		...
		public Control ControlToValidate
		{
			...
			set
			{
				...
				// Hook up ControlToValidates Validating event at run-time ie not from VS.NET
		...
		public void Validate()
		{
			...
			if (!_isValid)
			{
				if (_OrgBackColor == Color.Empty)
					_OrgBackColor = _controlToValidate.BackColor;
				_controlToValidate.BackColor = _HighlightColor;
				errorMessage = _errorMessage;
				_errorProvider.Icon = _icon;
			}
			else
			{
				_controlToValidate.BackColor = _OrgBackColor;
			}
		...
		{
			_HighlightColor = UIHelper.GetFormSettings((Control)sender).ErrorBackColor;
	...
	[ToolboxBitmap(typeof(RequiredFieldValidator), "RequiredFieldValidator.bmp")]
	public class RequiredFieldValidator : BaseValidator
	{
		...
		{
			if (!ControlToValidate.Enabled) return true;
	...
	[ToolboxBitmap(typeof(RegularExpressionValidator), "RegularExpressionValidator.bmp")]
	...
	[ToolboxBitmap(typeof(CustomValidator), "CustomValidator.bmp")]
	...
	[ToolboxBitmap(typeof(RangeValidator), "RangeValidator.bmp")]
	...
	[ToolboxBitmap(typeof(CompareValidator), "CompareValidator.bmp")]
	...
	[ToolboxBitmap(typeof(ContainerValidator), "ContainerValidator.bmp")]
	...
	[ToolboxBitmap(typeof(FormValidator), "FormValidator.bmp")]
	...
	[ToolboxBitmap(typeof(ValidationSummary), "ValidationSummary.bmp")]
	...
	public class ValidationSummary : BaseValidationSummary
	{
		...
		protected override void Summarize(object sender, SummarizeEventArgs e)
		{
			...
			// Dont validate if no validators were passed
	...
	[ToolboxBitmap(typeof(ListValidationSummary), "ListValidationSummary.bmp")]
	public class ListValidationSummary : BaseValidationSummary
	{
		BaseContainerValidator currentExtendee = null;

		private string _SummaryFormType;

		[Browsable(false)]
		public string SummaryFormType
		{
			get { return _SummaryFormType; }
			set { _SummaryFormType = value; }
		}	

		private BaseValidationSummaryForm _dlg;
		...
		protected override void Summarize(object sender, SummarizeEventArgs e)
		{
			...
			if (_dlg == null)
			{
				...
				_dlg = (BaseValidationSummaryForm)Activator.CreateInstance(ass.GetType(_SummaryFormType));
				_dlg.SetError(GetErrorCaption(extendee), GetErrorMessage(extendee));
