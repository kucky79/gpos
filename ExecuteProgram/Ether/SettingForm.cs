...
namespace Jarvis
{
    public partial class SettingForm : Form
    {
        ...
        private void TxtAdminPassword_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                btnAdminCheck.PerformClick();
            }
        }

        private void BtnAdminCheck_Click(object sender, EventArgs e)
        {
            object PasswordAdmin = DBHelper.ExecuteScalar("SELECT NO_PWD_ADMIN FROM MAS_FIRM WHERE CD_FIRM = '1000'");

            if(txtAdminPassword.Text == PasswordAdmin.ToString())
            {
                panelAdmin.Visible = false;
            }
            else
            {
                MessageBox.Show("암호가 정확하지 않습니다.");
        ...
        private void InitSetting()
        {
            ...
            //Connection
            ConnectionString = encryptor.Encryptor(DesType.Decrypt, inifile.IniReadValue("DB", "ConnectionString", Path));

            if (ConnectionString == string.Empty)
            {
                //default 
                inifile.IniWriteValue("DB", "ConnectionString", encryptor.Encryptor(DesType.Encrypt, "Data Source=localhost;initial Catalog=GPOS;uid=YOUR_USER;password=YOUR_PASSWORD;"), Path);
                ConnectionString = encryptor.Encryptor(DesType.Decrypt, inifile.IniReadValue("DB", "ConnectionString", Path));
            }
