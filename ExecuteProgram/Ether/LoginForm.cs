...
namespace Jarvis
{
    public partial class LoginForm : Form
    {
        ...
        private void LoginForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (!(loginStatus == UserLoginStatus.LogInSuccess || loginStatus == UserLoginStatus.Warning))
            {
                CloseConfirmDialog cfd = new CloseConfirmDialog();
                cfd.OwnerForm = this;
        ...
        private void DoLogin(object sender)
        {
            ...
            try
            {
                ...
                string _StoreCode = inifile.IniReadValue("StoreInfo", "StoreCode", _path);
                if (_StoreCode == string.Empty)
                {
                    //default 
                    inifile.IniWriteValue("StoreInfo", "StoreCode", "SE0001", _path);
                    _StoreCode = inifile.IniReadValue("StoreInfo", "StoreCode", _path);
                }

                string _LanguageCode = inifile.IniReadValue("StoreInfo", "Language", _path);
                if (_LanguageCode == string.Empty)
                {
                    //default 
                    inifile.IniWriteValue("StoreInfo", "Language", "KO", _path);
                    _LanguageCode = inifile.IniReadValue("StoreInfo", "Language", _path);
                }

                string encodedUserID = Base.BifrostEncrypt(txtUserID.Text);
                string encodedPassword = Base.BifrostEncrypt(txtPassword.Text);

                loginInfo = SystemMgr.AuthenticatePOS(_StoreCode, _LanguageCode, encodedUserID, encodedPassword, sIPAddr, sMACAdr);
                ...
                switch (loginStatus)
                {
                    ...
                    case UserLoginStatus.LogInSuccess:
                        ...
                        inifile.IniWriteValue("StoreInfo", "UserID", txtUserID.Text, _path);
                        ...
                        /// 
                        ev = new LoggedInEventArgs(txtUserID.Text, txtPassword.Text, loginInfo.Tables["UserInfo"].Rows[0]);
                        OnLoggedIn(this, ev);
                    ...
                    case UserLoginStatus.Warning:
                        ...
                        if (ShowMessageBox(loginInfo.Tables["USerInfo"].Rows[0]["DC_MESSAGE"].ToString(), MessageType.Warning) == DialogResult.OK)
                        {
                            inifile.IniWriteValue("StoreInfo", "UserID", txtUserID.Text, _path);
                            this.Hide();
        ...
        {
            txtUserID.Text = inifile.IniReadValue("StoreInfo", "UserID", _path);
